...
export async function add_heading({ text, level = 'Heading 1' }) {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function underline() {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function create_list({ type = 'bulleted' }) {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function insert_link({ url, text }) {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function find_replace({ find, replace }) {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function set_font_size({ size }) {
  try {
...
export async function insert_table({ rows, cols }) {
  try {
    const editor = getContentEditable();
    if (!editor) return 'editor_not_found';
...
export async function add_comment({ text }) {
  try {
...
export async function set_page_color({ color }) {
  try {
    const colorMap = {
      'white': '#ffffff',
      'light_gray': '#f5f5f5',
